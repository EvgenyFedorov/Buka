...
class BusinessController extends Controller implements IUsersRole{
    ...
    public $count_file_limit = 4;
    public $file_dir = 'adverts_businessman/';
    ...
    public function passwordsIdentical($input){
        ...
        return ($input['passw_business'] == $input['passw2_business']) ? true : false;
    ...
    public function initAdvertCreate(){
        ...
        return view('businessman.advert-create', [
            //'projects' => $projects,
    ...
    public function initAdverts(){
        ...
        return view('businessman.adverts', [
            'projects' => $projects,
            'count_dialogs' => $count_dialogs,
            'count_adverts' => $count_adverts
        ]);
    ...
    public function initAdvertsOnId($idp){
        ...
        $activity_type = $this->getActivityTypeOnId($projects[0]->id_business_activity);
        $sub_activity_type = $this->getSubActivityContent($projects[0]->id_business_sub_activities);

        $count_adverts = $this->getCountProjects();
        $count_dialogs = $this->getCountDialogs();

        return view('businessman.advert-one', [
            'projects' => $projects[0],
            'count_adverts' => $count_adverts,
            'count_dialogs' => $count_dialogs,
            'activity' => $activity_type[0],
            'sub_activity' => $sub_activity_type,
            'imgs' => isset($img[0]) ? $img : false
        ]);
    ...
    public function initDialogs(){
        ...
        return view('businessman.dialogs', [
            'dialogs' => $dialogs,
            'count_adverts' => $count_adverts,
            'count_dialogs' => $count_dialogs
        ]);
    ...
    public function initDialogsOnId($idd){
        ...
        return view('businessman.dialog-one', [
            'dialog' => $dialog,
            'count_adverts' => $count_adverts,
            'count_dialogs' => $count_dialogs,
    ...
    public function initPaymentAdvertOnId($idp){
        ...
        return view('businessman.advert-payment', [
            'projects' => $projects[0],
            'count_adverts' => $count_adverts,
            'count_dialogs' => $count_dialogs,
        ]);
    ...
    public function initSetting(){
        ...
        return view('businessman.setting', [
    ...
    public function initProfile(){
        ...
        return view('businessman.profile', [
            'count_adverts' => $count_adverts,
            'count_dialogs' => $count_dialogs,
        ]);
    ...
    public static function getImgs($projects, $uid){

        $imgs = array();
        $images = json_decode($projects[0]->img_business_proj);

        for($i=0; $i <= count($images); $i++){

            if(!empty($images[$i])){

                $return = LoadFileImg::getImgOnPath(
                    $images[$i],
                    $uid
                );

                if(isset($return[0]) AND !empty($return[0])){
    ...
    public function getProjects(){
        ...
        return FevBusinessmanProject::loadProjects([
            ['id_user_business_proj', $this->user->id]
        ],
    ...
    public function getCountProjects(){
        ...
        // 1 аргумент - where
        return FevBusinessmanProject::loadCountProjects([
            ['id_user_business_proj', $this->user->id]
        ]);
    ...
    public function getProjectOnId($idp){
        ...
        return FevBusinessmanProject::loadProject([
            ['id_user_business_proj', $this->user->id],
            ['id_business_proj', $idp],
        ],
            'id_business_proj');
    ...
    public function setProject($input){
        ...
        $this->advert_id = FevBusinessmanProject::addProject($input);
    ...
    public function updateProject($input){
        ...
        $input['id_user'] = $this->user->id;
        $input['sub_activity'] = str_replace('[', '', str_replace(']', '', $input['sub_activity']));
        $this->advert_id = FevBusinessmanProject::updateProject($input);
    ...
    public function updateProjectImg($input){
        ...
        $input['id_user'] = $this->user->id;
        $this->advert_id = FevBusinessmanProject::updateProjectImg($input);
    ...
    public function updateProjectStatus($input, $status){
        ...
        $input['id_user'] = $this->user->id;
        $input['status'] = $status;
        $this->advert_id = FevBusinessmanProject::updateProjectStatus($input);
    ...
    public function updateProjectFeedBacks($input){
        ...
        $this->advert_id = FevBusinessmanProject::updateProjectFeedBack($input);
    ...
    public function registration($input){
        ...
        $registration_user_id = FevUser::createUser(array(
            'email' => $input['mail_business'],
            'password' => $input['passw_business'],
            'confirm_link' => $email->data_email['confirm_link'],
            'invite_code' => $invite_code,
            'user_ranks' => $input['tarif_business'],
            'id_group' => 4,
        ));
        ...
        $add_user_info = FevUser::addUserInfo(array(
            ...
            'fname' => $input['fname_business'],
            'user_phone' => $input['phone_business'],
